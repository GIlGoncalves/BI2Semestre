setwd("/Users/josepedromonteiro/GitHub/BI2Semestre/")
#Pre processamento dos dados

# I. Leitura do dataset inicial
dataset<-read.csv("outMovieFinal.csv",header = T)

# 1. Numero de campos e de variáveis
print("Numero de campos e de variáveis:")
...
# II. Problema dos NulosVerificar os NAs no dataset

print("Numero de NA's:")
sum(is.na(dataset))

#1. Verificar se uma celula tem elemntos vazios
isCellEmpty = function(cell)
{
  as.character(cell) == ""
}

isCellEmpty(dataset$director_name)



# Calcula o numero de celulas sem valor
...
# Verifica o número de celulas negativas neste dataset 
print("Número de celulas negativas neste dataset:")
numEmptyCellsInDataFrame(dataset) 



## III - Problema dos duplicados
## Tem filmes duplicados,porem n~ão podem ser completamente removidos
## Pq pode haver diferentes diretores com nomes de filmes iguais
## e diferentes numeros de votos

# Numero de duplicados
print("Numero de duplicados:")
sum(duplicated(dataset)) #45

# Numero de titulos duplicados
print("Numero de titulo duplicados:")
sum(duplicated(dataset$movie_title)) #126


# Numero de titulos duplicados com o diretor
print("Numero de titulos duplicados com o diretor:")
sum(duplicated(dataset[,c("movie_title", "director_name")])) #124

# Numero de titulos duplicados com numero de votos dos utilizadores
print("Numero de titulos duplicados com numero de votos dos utilizadores:")
sum(duplicated(dataset[,c("movie_title", "num_voted_users")])) #47

#1 of c in dataset Genero
for(i in 29:length(dataset)) {
...
onlyNumerical<-dataset[,c(-4,-16,-17,-18,-22,-27,-10)]
...
barplot(actor_3_facebook_likes1$Freq,names.arg = actor_3_facebook_likes1$Var1,main="N?mero de likes do actor 3",xlab = "N?mero",col = c("yellow","blue"))
dev.off()
###############################################################################################
...
barplot(actor_1_facebook_likes1$Freq,names.arg = actor_1_facebook_likes1$Var1,main="N?mero de likes do actor 1",xlab = "N?mero",col = c("yellow","blue"))
dev.off()
...
barplot(actor_2_facebook_likes1$Freq,names.arg = actor_2_facebook_likes1$Var1,main="N?mero de likes do actor 2",xlab = "N?mero",col = c("yellow","blue"))
dev.off()
